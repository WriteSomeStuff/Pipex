...
/*   Updated: 2023/04/05 18:32:32 by cschabra      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_lastcmd(char **argv, int32_t lastcmd, t_file *file)
{
	if (close(file->fd1[1]) == -1)
	...
	while (i <= lastcmd)
	{
		...
		if (pid->pid == 0)
		{
			if (i == 2)
				...
				ft_lastcmd(arg->argv, lastcmd, file);
